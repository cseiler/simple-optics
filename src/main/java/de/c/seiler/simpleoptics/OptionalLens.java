...
public class OptionalLens<A, B> extends OptionalView<A, B> {
	...
	public OptionalLens(Function<Optional<A>, Optional<B>> fget,
			BiFunction<Optional<A>, Optional<B>, Optional<A>> fset) {
		super(fget);
		this.fset = fset;
	}
	...
	public <C> OptionalLens<C, B> compose(final Lens<C, A> that) {
		return new OptionalLens<C, B>(c -> c.flatMap(c1 -> get(that.get(c1))), (c, b) -> c
				.flatMap(c2 -> b.map(b1 -> that.mod(c2, a -> set(a, b1).orElseThrow(NullPointerException::new)))));
	}

	public <C> OptionalLens<C, B> compose(final OptionalLens<C, A> that) {
		return new OptionalLens<C, B>(c -> get(that.get(c)), (c, b) -> that.mod(c, a -> set(a, b)));
	}
