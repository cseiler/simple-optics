...
public class OptionalStreamView<A, B> extends OptionalView<A, Stream<B>> {
	...
	public <C> OptionalStreamView<C, B> composeFlatMap(final StreamView<C, A> that) {
		return new OptionalStreamView<C, B>(
				c -> c.map(cc -> that.get(cc).map(Optional::of).map(fget).flatMap(os -> os.orElseGet(Stream::empty))));
	}

	public <C> OptionalStreamView<C, B> composeFlatMap(final OptionalStreamView<C, A> that) {
		return new OptionalStreamView<C, B>(c -> that.get(c).map(sta -> sta.map(Optional::of))
				.map(stoa -> stoa.map(fget)).map(sosb -> sosb.flatMap(osb -> osb.orElseGet(Stream::empty))));
	}
