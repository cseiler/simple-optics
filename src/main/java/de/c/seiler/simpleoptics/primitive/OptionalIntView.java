...
public class OptionalIntView<A> extends OptionalView<A, Integer> {
	...
	public OptionalIntView(IntView<A> lens) {
		super(oa -> oa.isPresent() ? Optional.of(Integer.valueOf(lens.fgetInt.applyAsInt(oa.get())))
				: Optional.empty());
	...
	public <C> OptionalIntView<C> compose(final View<C, A> that) {
		return new OptionalIntView<C>(oc -> oc.isPresent() ? getAsInt(that.get(oc.get())) : OptionalInt.empty());
	}
