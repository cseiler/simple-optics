...
public class OptionalDoubleStreamView<A> extends OptionalView<A, DoubleStream> {
	...
	public <C> OptionalDoubleStreamView<C> composeFlatMap(final StreamView<C, A> that) {
		return new OptionalDoubleStreamView<C>(c -> c.map(cc -> that.get(cc).map(Optional::of).map(fget)
	...
	public <C> OptionalDoubleStreamView<C> composeFlatMap(OptionalStreamView<C, A> that) {
		return new OptionalDoubleStreamView<C>(
				c -> that.get(c).map(sta -> sta.map(Optional::of)).map(stoa -> stoa.map(fget))
						.map(sosb -> sosb.flatMapToDouble(osb -> osb.orElseGet(DoubleStream::empty))));
	}
}
