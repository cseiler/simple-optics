...
public class OptionalLongView<A> extends OptionalView<A, Long> {
	...
	public OptionalLongView(LongView<A> lens) {
		...
		this.fgetLong = oa -> oa.isPresent() ? OptionalLong.of(lens.fgetLong.applyAsLong(oa.get()))
				: OptionalLong.empty();
	}
	...
	public <C> OptionalLongView<C> compose(final View<C, A> that) {
		return new OptionalLongView<C>(oc -> oc.isPresent() ? getAsLong(that.get(oc.get())) : OptionalLong.empty());
	}
