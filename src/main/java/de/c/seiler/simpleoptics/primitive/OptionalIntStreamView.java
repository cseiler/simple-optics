...
public class OptionalIntStreamView<A> extends OptionalView<A, IntStream> {
	...
	public <C> OptionalIntStreamView<C> composeFlatMap(final StreamView<C, A> that) {
		return new OptionalIntStreamView<C>(c -> c.map(
				cc -> that.get(cc).map(Optional::of).map(fget).flatMapToInt(os -> os.orElseGet(IntStream::empty))));
	}

	public <C> OptionalIntStreamView<C> composeFlatMap(OptionalStreamView<C, A> that) {
		return new OptionalIntStreamView<C>(c -> that.get(c).map(sta -> sta.map(Optional::of))
				.map(stoa -> stoa.map(fget)).map(sosb -> sosb.flatMapToInt(osb -> osb.orElseGet(IntStream::empty))));
	}
}
