...
public class OptionalIntLens<A> extends OptionalIntView<A> {
	...
	public OptionalIntLens(IntLens<A> lens) {
		...
		this.fset = (oa, ob) -> oa.map(a -> ob.isPresent() ? lens.set(a, ob.getAsInt()) : a);
	}
	...
	public Optional<A> mod(Optional<A> a, Function<OptionalInt, OptionalInt> f) {
		OptionalInt x = f.apply(getAsInt(a));
	...
	public <C> OptionalIntLens<C> compose(final Lens<C, A> that) {
		return new OptionalIntLens<C>(c -> c.isPresent() ? getAsInt(that.get(c.get())) : OptionalInt.empty(),
				(c, b) -> c.flatMap(c2 -> {
					if (b.isPresent())
						return Optional
	...
	public <C> OptionalIntLens<C> compose(final OptionalLens<C, A> that) {
		return new OptionalIntLens<C>(c -> getAsInt(that.get(c)), (c, b) -> that.mod(c, a -> set(a, b)));
	}
}