 ...
 * immutable data." Inspired by and expanded upon:
...
public class DoubleLens<A> extends DoubleView<A> {
	...
	public <C> DoubleLens<C> compose(final Lens<C, A> that) {
		return new DoubleLens<C>(c -> getAsDouble(that.get(c)), (c, b) -> that.mod(c, a -> set(a, b)));
	}

	public <C> OptionalDoubleLens<C> compose(final OptionalLens<C, A> that) {
		return new OptionalDoubleLens<C>(c -> {
