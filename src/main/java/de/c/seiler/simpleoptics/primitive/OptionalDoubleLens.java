...
public class OptionalDoubleLens<A> extends OptionalDoubleView<A> {
	...
	public OptionalDoubleLens(DoubleLens<A> lens) {
		super(oa -> oa.isPresent() ? OptionalDouble.of(lens.fgetDouble.applyAsDouble(oa.get()))
				: OptionalDouble.empty());
		this.fset = (oa, ob) -> oa.map(a -> ob.isPresent() ? lens.set(a, ob.getAsDouble()) : a);
	}
	...
	public Optional<A> mod(Optional<A> a, Function<OptionalDouble, OptionalDouble> f) {
		OptionalDouble x = f.apply(getAsDouble(a));
	...
	public <C> OptionalDoubleLens<C> compose(final Lens<C, A> that) {
		return new OptionalDoubleLens<C>(c -> c.isPresent() ? getAsDouble(that.get(c.get())) : OptionalDouble.empty(),
				(c, b) -> c.flatMap(c2 -> {
					if (b.isPresent())
						return Optional.of(
								that.mod(c2, a -> set(a, b.getAsDouble()).orElseThrow(NoSuchElementException::new)));
					return Optional.empty();
				}));
	}

	public <C> OptionalDoubleLens<C> compose(final OptionalLens<C, A> that) {
		return new OptionalDoubleLens<C>(c -> getAsDouble(that.get(c)), (c, b) -> that.mod(c, a -> set(a, b)));
	}
}