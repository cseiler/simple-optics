...
public class OptionalLongStreamView<A> extends OptionalView<A, LongStream> {
	...
	public <C> OptionalLongStreamView<C> composeFlatMap(final StreamView<C, A> that) {
		return new OptionalLongStreamView<C>(c -> c.map(
				cc -> that.get(cc).map(Optional::of).map(fget).flatMapToLong(os -> os.orElseGet(LongStream::empty))));
	}

	public <C> OptionalLongStreamView<C> composeFlatMap(OptionalStreamView<C, A> that) {
		return new OptionalLongStreamView<C>(c -> that.get(c).map(sta -> sta.map(Optional::of))
				.map(stoa -> stoa.map(fget)).map(sosb -> sosb.flatMapToLong(osb -> osb.orElseGet(LongStream::empty))));
	}
}
